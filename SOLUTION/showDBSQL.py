...
class initQuery:
	...
	def __init__ (self):
		...
		SHOW DATABASES		-> Lists all DB's in the system
		
		'''
		
		self.sStmt	= HDR.PARSER.Forward ()
		
		self.sKeyword01	= HDR.PARSER.Keyword ('SHOW DATABASES', caseless=True).setResultsName ('OPERATION')
	...
	def processQuery (self, psQuery=''):
		...
		try:
			...
			lTmp	= HDR.OS.listdir (HDR.OS.path.join (HDR.gsInstallPath, 'data'))
